...
-- 객체변경(수정) ALTER (TABLE, VIEW, INDEX)
...
-- 데이터 전체 조회
SELECT io_date, io_buyer, io_pname, io_qty, io_price
FROM tbl_iolist;
...
-- 기존의 Table을 제거하고 무결성을 유지하기 위한 "제약조건을 설정" 하여 Table 을 다시 만들자
...
-- 데이터를 INSERT하는데 필수적으로 setting 되어야 할 칼럼에 필수 제약조건을 설정하자
CREATE TABLE tbl_iolist(
...
-- 3. list를 출력한다.
SELECT io_date AS 거래일자, io_buyer AS 고객명, io_pname AS 상품명, io_qty AS 수량, io_price AS 단가, io_qty * io_price AS 합계
FROM tbl_iolist;

INSERT INTO tbl_iolist(io_date,io_buyer,io_pname,io_qty,io_price)
VALUES('2021-04-21','이몽룡','고래밥',10,800);
INSERT INTO tbl_iolist(io_date,io_buyer,io_pname,io_qty,io_price)
VALUES('2021-04-21','이성계','포테토칩',80,1000);
INSERT INTO tbl_iolist(io_date,io_buyer,io_pname,io_qty,io_price)
VALUES('2021-04-21','강감찬','포스틱',600,1200);
INSERT INTO tbl_iolist(io_date,io_buyer,io_pname,io_qty,io_price)
VALUES('2021-04-21','최건진','포카칩',120,1200);

SELECT * FROM tbl_iolist;

-- 1. tbl_iolist로 부터 데이터를 가져오기
-- 2. 가져온 데이터 중에서 io_buyer 칼럼에 저장된 값이 '홍길동' 인 데이터만 간추려서 보여달라
SELECT *
FROM tbl_iolist
WHERE io_buyer = '홍길동';


-- tbl_iolist에 저장되어 있는 데이터 리스트중에서 io_buyer 칼럼의 값이 ' 홍길동' 인 데이터만 간추려서
-- io_buyer, io_pname 칼럼 보이고 나머지 칼럼은 숨김으로 하여 보여달라
-- 보여지는 칼럼에는 별명을 붙여서 보여달라
SELECT io_buyer 고객이름, io_pname 구매상품
FROM tbl_iolist
WHERE io_buyer = '홍길동';
